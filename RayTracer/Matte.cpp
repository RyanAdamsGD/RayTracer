...
Matte::Matte(void)
	:Material(),ambientBrdf(new Lambert),diffuseBrdf(new Lambert)
{
}

void Matte::setKAmbient(float k)
{
	ambientBrdf->setReflectionCoefficient(k);
}

void Matte::setKDiffuse(float k)
{
	diffuseBrdf->setReflectionCoefficient(k);
}

void Matte::setColor(const glm::vec3& color)
{
	diffuseBrdf->setColor(color);
	ambientBrdf->setColor(color);
}

glm::vec3 Matte::shade(const ShadeRec& sr)const
{
