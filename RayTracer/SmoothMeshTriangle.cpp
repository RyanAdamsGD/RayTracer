...
glm::vec3 SmoothMeshTriangle::interpolateNormals(float beta, float gamma)const
{
	glm::vec3 norm((1.0f - beta - gamma) * mesh->normals[index0] + 
		(float)beta * mesh->normals[index1] + (float)gamma * mesh->normals[index2]);
...
bool SmoothMeshTriangle::hit(const Ray& ray, float& tmin, ShadeRec& sr) const
{
	...
	tmin = t;
	sr.normal = interpolateNormals(beta,gamma);
