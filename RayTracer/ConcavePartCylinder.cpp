...
ConcavePartCylinder::ConcavePartCylinder(float y0, float y1, float radius, float azimuthMax,float azimuthMin)
	:y0(y0),y1(y1),radius(fabs(radius)),inverseRadius(1.0f/radius),azimuthMax(azimuthMax), azimuthMin(azimuthMin)
{
	bbox = createBoundingBox();
}

BoundingBox ConcavePartCylinder::createBoundingBox()const
{
...
BoundingBox ConcavePartCylinder::getBoundingBox()const
{
	return bbox;
}
...
bool ConcavePartCylinder::hit(const Ray& ray, float& tmin, ShadeRec& sr) const
{
	float dx = ray.getDirection().x; float dz = ray.getDirection().z;
	float ox = ray.getOrigin().x; float oz = ray.getOrigin().z;
...
void ConcavePartCylinder::setY0(float value) 
{
	y0=value;
	bbox = createBoundingBox();
}

void ConcavePartCylinder::setY1(float value) 
{
	y1=value;
	bbox = createBoundingBox();
}

void ConcavePartCylinder::setRadius(float value) 
{
	radius=fabs(value);
